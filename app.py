...
# Version 4: Ausgabeformat entspricht der freigegebenen Referenzdatei.
...
def format_header(doc, ort, niveau, pruefungsnummer, datum, farbe):
    # In diesen Vorlagen bestehen die sichtbaren Kopfzeilen aus 2 Word-Absätzen:
    # Absatz 1 = Ort
    # Absatz 2 = Niveau + Prüfungsnummer + Datum/Farbe, jeweils in eigener Zeile
    visible_paragraphs = [p for p in doc.paragraphs if p.text.strip()]

    if len(visible_paragraphs) < 2:
